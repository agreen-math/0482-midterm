...
def parse_checkit_item(raw_block):
    """
    Parses a top-level CheckIt item using the Midterm brace-matching logic.
...
def extract_tikz(text):
    """
    Uses regex to capture the TikZ picture environment in the block.
    """
    match = re.search(r'\\begin\{tikzpicture\}.*?\\end\{tikzpicture\}', text, re.DOTALL)
...
def main():
    parser = argparse.ArgumentParser(description="CheckIt to Department LaTeX Parser - Comprehensive Final")
    ...
    # Safely isolate the 21 top-level CheckIt items
    ...
    # Validate that we successfully parsed exactly 21 items
    q_items = []
    for i in range(21):
    ...
    # Map the 21 Items directly to the 22 Template Questions
    
    # Q1
    q1 = r"\question[5] Simplify.\\ \\ " + get_math(q_items[0]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[0]['solution'])
    questions.append(q1)

    # Q2
    q2 = r"\question[5] Simplify.\\ \\ " + get_math(q_items[1]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[1]['solution'])
    questions.append(q2)

    # Q3
    q3 = r"\question[5] Simplify.\\ \\ " + get_math(q_items[2]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[2]['solution'])
    questions.append(q3)

    # Q4
    q4 = r"\question[5] Simplify.\\ \\ " + get_math(q_items[3]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[3]['solution'])
    questions.append(q4)

    # Q5
    q5 = r"\question[5] Solve for all solutions.\\ \\ " + get_math(q_items[4]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[4]['solution'])
    questions.append(q5)

    # Q6
    q6 = r"""\question[5] Graph and identify the slope and $y$-intercept: \\

""" + get_math(q_items[5]['content'], 0) + r"""
...
\end{multicols}""" + format_solution(q_items[5]['solution'])
    questions.append(q6)

    # Q7 and Q8 (The Multi-Part Graphic Question from Item 6)
    sub0 = q_items[6]['sub_items'][0] if q_items[6]['type'] == 'multi' and len(q_items[6]['sub_items']) > 0 else {'content':'', 'solution':''}
    sub1 = q_items[6]['sub_items'][1] if q_items[6]['type'] == 'multi' and len(q_items[6]['sub_items']) > 1 else {'content':'', 'solution':''}

    inst_func = r"\uplevel{\textbf{For Problems \ref{func-start} and \ref{func-end}, identify the domain and range and determine whether or not the graph represents a function. if it is not a function, explain why.}}" + "\n\n"
    
    q7 = inst_func + r"""\question[4]\label{func-start} \,
...
\columnbreak
\centering
""" + extract_tikz(sub0['content']) + r"""

\end{multicols}""" + format_solution(sub0['solution'])
    questions.append(q7)

    q8 = r"""\question[4]\label{func-end} \,
...
\columnbreak
\centering
""" + extract_tikz(sub1['content']) + r"""

\end{multicols}""" + format_solution(sub1['solution'])
    questions.append(q8)

    # Q9
    q9 = r"\question[4] Find the equation of the line \textbf{parallel} to " + get_math(q_items[7]['content'], 0) + r" and passing through " + get_math(q_items[7]['content'], 1) + r"." + "\n" + r"\vspace*{\stretch{2}}\answerline" + format_solution(q_items[7]['solution'])
    questions.append(q9)

    # Q10
    q10 = r"\question[5] Solve.\\ \\ " + get_math(q_items[8]['content'], 0) + "\n" + r"\vspace*{\stretch{2}}\answerline" + format_solution(q_items[8]['solution'])
    questions.append(q10)

    # Q11
    q11 = r"\question[4] Solve for all solutions.\\ \\ " + get_math(q_items[9]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[9]['solution'])
    questions.append(q11)

    # Q12
    q12 = r"\question[5] Given " + get_math(q_items[10]['content'], 0) + r", simplify " + get_math(q_items[10]['content'], 1) + r" when " + get_math(q_items[10]['content'], 2) + r"." + "\n" + r"\vspace*{\stretch{2}}\answerline" + format_solution(q_items[10]['solution'])
    questions.append(q12)

    # Q13
    q13 = r"\question[5] Solve for all solutions. \\ \\ " + get_math(q_items[11]['content'], 0) + "\n" + r"\vspace*{\stretch{2}}\answerline" + format_solution(q_items[11]['solution'])
    questions.append(q13)

    # Q14
    q14 = r"\question[4] Simplify the radical expression.\\ \\ " + get_math(q_items[12]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[12]['solution'])
    questions.append(q14)

    # Q15
    inst_rad1 = r"\uplevel{\textbf{For Problems \ref{rad-simp-begin} through \ref{rad-simp-end}, perform the operations and simplify. Assume all variables are positive and rationalize the denominator when appropriate.}}" + "\n\n"
    q15 = inst_rad1 + r"\question[5]\label{rad-simp-begin} " + get_math(q_items[13]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[13]['solution'])
    questions.append(q15)

    # Q16
    inst_rad2 = r"\uplevel{\textbf{For Problems \ref{rad-simp-begin} through \ref{rad-simp-end}, perform the operations and simplify. Assume all variables are positive and rationalize the denominator when appropriate.}}" + "\n\n"
    q16 = inst_rad2 + r"\question[4] " + get_math(q_items[14]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[14]['solution'])
    questions.append(q16)

    # Q17
    q17 = r"\question[4]\label{rad-simp-end} " + get_math(q_items[15]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[15]['solution'])
    questions.append(q17)

    # Q18
    q18 = r"\question[4] Solve for all solutions.\\ \\ " + get_math(q_items[16]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[16]['solution'])
    questions.append(q18)

    # Q19
    q19 = r"\question[5] Solve for all solutions.\\ \\ " + get_math(q_items[17]['content'], 0) + "\n" + r"\vspace*{\stretch{2.5}}\answerline" + format_solution(q_items[17]['solution'])
    questions.append(q19)

    # Q20
    q20 = r"\question[4] Solve for all solutions.\\ \\ " + get_math(q_items[18]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[18]['solution'])
    questions.append(q20)

    # Q21
    q21 = r"\question[5] Solve for all solutions.\\ \\ " + get_math(q_items[19]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[19]['solution'])
    questions.append(q21)

    # Q22
    q22 = r"\question[4] Solve using the quadratic formula.\\ \\ " + get_math(q_items[20]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[20]['solution'])
    questions.append(q22)

    # Apply exact page breaks matching Temp-Comp-Final.tex
    page_breaks = [1, 3, 5, 7, 9, 11, 13, 14, 16, 18, 20]
