\documentclass[addpoints,14pt,answers]{exam}
...
\uplevel{\textbf{\hspace*{-0.26in}For Problems \ref{func-start} and \ref{func-end}, identify the domain and range and determine whether or not the graph represents a function.  if it is not a function, explain why.}}
...
\columnbreak
\centering
\begin{tikzpicture}[scale=0.35] \draw[step=1cm, gray!40, very thin] (-10,-10) grid (10,10); \draw[thick, <->] (-10.5,0) -- (10.5,0) node[right] {$x$}; \draw[thick, <->] (0,-10.5) -- (0,10.5) node[above] {$y$}; \foreach \x in {-10,-8,...,10} \draw (\x, 0.1) -- (\x, -0.1) node[below] {\tiny $\x$}; \foreach \y in {-10,-8,...,10} \draw (0.1, \y) -- (-0.1, \y) node[left] {\tiny $\y$}; \draw[ultra thick, blue, <->, domain=-3.0990195135927845:7.0990195135927845, samples=100] plot ({0.500000000000000*(\x - 2)^2 + -3}, \x); \end{tikzpicture}
...
\columnbreak
\centering
\begin{tikzpicture}[scale=0.35] \draw[step=1cm, gray!40, very thin] (-10,-10) grid (10,10); \draw[thick, <->] (-10.5,0) -- (10.5,0) node[right] {$x$}; \draw[thick, <->] (0,-10.5) -- (0,10.5) node[above] {$y$}; \foreach \x in {-10,-8,...,10} \draw (\x, 0.1) -- (\x, -0.1) node[below] {\tiny $\x$}; \foreach \y in {-10,-8,...,10} \draw (0.1, \y) -- (-0.1, \y) node[left] {\tiny $\y$}; \draw[ultra thick, blue, <->, domain=-2.79141985706278:6.48140474655716, samples=100] plot (\x, {0.100000000000000*(\x - 2)^3 + 1}); \end{tikzpicture}
...
\uplevel{\textbf{\hspace*{-0.26in}For Problems \ref{rad-simp-begin} through \ref{rad-simp-end}, perform the operations and simplify.  Assume all variables are positive and rationalize the denominator when appropriate.}}
