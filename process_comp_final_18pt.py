...
def main():
    parser = argparse.ArgumentParser(description="CheckIt to Department LaTeX Parser - Comprehensive Final (18pt)")
    ...
    # Q6 (18pt format - no multicols, larger graph)
...
""" + format_solution(q_items[5]['solution'])
    questions.append(q6)

    # Q7 and Q8 (The Multi-Part Graphic Question from Item 6 - Centered layout for 18pt)
    ...
    inst_func = r"\uplevel{\textbf{\hspace*{-0.4in}For Problems \ref{func-start} and \ref{func-end}, identify the domain and range and determine whether or not the graph represents a function. if it is not a function, explain why.}}" + "\n\n"
...
\begin{tabular}{|c|p{1.5in}|}\hline
    ...
    Range & \\ \hline
\end{tabular}}
""" + format_solution(sub0['solution']) + r"\vspace{\stretch{1}}"
    questions.append(q7)

    q8 = inst_func + r"""\question[4]\label{func-end}
\,
\begin{center}
...
\begin{tabular}{|c|p{1.5in}|}\hline
    ...
    Range & \\ \hline
\end{tabular}}
""" + format_solution(sub1['solution']) + r"\vspace{\stretch{1}}"
    questions.append(q8)
    ...
    # Q15
    inst_rad = r"\uplevel{\textbf{\hspace*{-0.4in}For Problems \ref{rad-simp-begin} through \ref{rad-simp-end}, perform the operations and simplify. Assume all variables are positive and rationalize the denominator when appropriate.}}" + "\n\n"
    q15 = inst_rad + r"\question[5]\label{rad-simp-begin} " + get_math(q_items[13]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[13]['solution'])
    questions.append(q15)

    # Q16
    q16 = inst_rad + r"\question[4] " + get_math(q_items[14]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[14]['solution'])
    questions.append(q16)

    # Q17
    q17 = inst_rad + r"\question[4]\label{rad-simp-end} " + get_math(q_items[15]['content'], 0) + "\n" + r"\vspace*{\stretch{1}}\answerline" + format_solution(q_items[15]['solution'])
    ...
    # Apply exact page breaks matching Temp-Comp-Final-18pt.tex (1 question per page)
    for idx in range(21):
